...
class HistoryList extends Component {
  ...
  render() {
    ...
    const matchingHistoryList = historyList.filter(eachItem => {
      const temp = eachItem.title.toLowerCase()
      const temp2 = searchInput.toLowerCase()
      return temp.includes(temp2)
    })
    const length = this.getListLength(matchingHistoryList)
    ...
    return (
      <div className="app-container">
        ...
        {length !== 0 && (
          <div className="history-items-container">
            <ul className="history-items-list">
              {matchingHistoryList.map(eachItem => (
                <HistoryItem
        ...
        {length === 0 && (
          <div className="no-items-list-container">
            <p className="no-items-list-text">There is no history to show</p>
          </div>
        )}
      </div>
    )
